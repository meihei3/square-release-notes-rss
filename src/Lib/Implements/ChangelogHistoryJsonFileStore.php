...
final readonly class ChangelogHistoryJsonFileStore implements ChangelogHistoryFileStoreInterface
{
    ...
    private const string REQUIREMENTS_JSON_FILE = '/json/requirements.json';
    ...
    public function storeRequirements(array $changelogHistories): void
    {
        ...
        $success = file_put_contents($this->publicDirectory . self::REQUIREMENTS_JSON_FILE, $json);
        if ($success === false) {
            throw new RuntimeException('Failed to write JSON to file');
        }
    }
    ...
    {
        $json = file_get_contents($this->publicDirectory . self::REQUIREMENTS_JSON_FILE);
        if ($json === false) {
            throw new RuntimeException('Failed to read JSON from file');
        }
